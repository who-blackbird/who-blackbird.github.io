...
**NB.: This command will take about 10 minutes, so it is a good time to have a break and stretch a bit your legs.** If the script is taking too long there is a hidden file that you can use to keep going. Do:
...
Bisulphite is nowadays commonly used to detect 5mC genome-wide. We can compare the results we got from the Nanopore methylation to the bisulphite doing. We could compare the score the softwares gave to the 5mC and the regions where they identified the modification. To compare the scores we could do:
